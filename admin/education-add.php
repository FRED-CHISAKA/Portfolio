<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        $v0 = post_string('degree');
        $v1 = post_string('institution');
        $v2 = post_string('location');
        $v3 = post_int('start_year');
        $v4 = post_int('end_year');
        $v5 = post_string('status');
        $v6 = $user_id;

        $sql = "INSERT INTO education (degree, institution, location, start_year, end_year, status, user_id) VALUES (?,?,?,?,?,?,?)";
        $stmt = mysqli_prepare($conn, $sql);

        if (!$stmt) {
            redirect_resume("Unable to prepare the education record.", "danger");
        }

        mysqli_stmt_bind_param($stmt, "sssiisi", $v0, $v1, $v2, $v3, $v4, $v5, $v6);

        if (mysqli_stmt_execute($stmt)) {
            mysqli_stmt_close($stmt);
            redirect_resume("Add Education added successfully.");
        }
    ...
    <div class="content">
        ...
        <div class="card shadow-sm border-0 mt-3">
            ...
            <div class="card-header bg-white py-3">
                <h3 class="h5 mb-1"><i class="bi bi-mortarboard me-2"></i>Add Education</h3>
