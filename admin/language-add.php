<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        $v0 = post_string('name');
        $v1 = post_string('proficiency');

        $sql = "INSERT INTO languages (name, proficiency) VALUES (?,?)";
        $stmt = mysqli_prepare($conn, $sql);

        if (!$stmt) {
            redirect_resume("Unable to prepare the language record.", "danger");
        }

        mysqli_stmt_bind_param($stmt, "ss", $v0, $v1);

        if (mysqli_stmt_execute($stmt)) {
            mysqli_stmt_close($stmt);
            redirect_resume("Add Language added successfully.");
        }
    ...
    <div class="content">
        ...
        <div class="card shadow-sm border-0 mt-3">
            <div class="card-header bg-white py-3">
                <h3 class="h5 mb-1"><i class="bi bi-translate me-2"></i>Add Language</h3>
